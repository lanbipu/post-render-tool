...
2. ``official_sensor_inverse_uv`` + ``map_center_shift_projection`` (Path C · 新路, Custom Post-Process Material):
   ``official_sensor_inverse_uv`` 照搬 docs/custom-postprocess-distortion-final-plan.md §2.4 的
   HLSL 公式, shader / C++ controller 必须照抄. ``map_center_shift_projection`` 把 Disguise CSV
   ``centerShiftMM`` 翻成 UE Filmback 投影偏移 + 材质 ``CenterUV``: X 轴直接透传 mm 进
   ``Filmback.Sensor*Offset``, Y 轴反号 (NDC↑ vs UE 屏幕↓). 公式与 focal 无关, 由 2026-05-07
   K=0 控制帧 phase-correlate 直接定型 (cs ±0.5mm → ±27.5px @ sensor 35mm × 1920×1080,
   残差 < 0.6%). 详见 docs/distortion-investigation.md "2026-05-07 — K=0 直接测量".
   Gate 1 单元测试守住两个函数的形态契约.
...
class CenterShiftProjectionMapping:
    """Path C mapping for one Disguise ``centerShiftMM`` sample.

    ``sensor_*_offset_mm`` drives UE CineCamera projection through
    ``Filmback.SensorHorizontalOffset`` / ``Filmback.SensorVerticalOffset``.
    ``center_u`` / ``center_v`` drive the post-process material's radial
    distortion center. Keeping both values together prevents the production
    pipeline and validation scripts from drifting apart.
    """

    center_u: float
    center_v: float
    sensor_horizontal_offset_mm: float
    sensor_vertical_offset_mm: float
...
def map_center_shift_projection(
    *,
    ...
    D3 的 ``centerShiftMM`` 是「光心相对传感器中心偏移多少 mm」, 跟 focal 无关.
    UE ``Filmback.SensorHorizontalOffset`` / ``SensorVerticalOffset`` 也是 mm 单位
    的光心偏移, 但**两个引擎对"偏"方向的定义在两个轴上都相反** (UE +X mm 让
    画面向 -X 移; D3 cs.x=+0.5 让画面向 +X 移). 所以两轴都取反号.

    公式 (2026-05-07 K=0 控制帧渲染闭环验证, X 残差 < 0.1 px, Y 残差 < 0.1 px):
    ...
    实测: cs ±0.5mm → 画面平移 ±27.5px (sensor 35mm, image 1920×1080,
    focal=30.302mm 但跟公式无关). UE 写 ∓cs mm 进 Filmback → UE 内部按
    ``image_dim/sensor_dim`` 转 px → 画面位移跟 D3 同方向同幅度. 详见
    docs/distortion-investigation.md "2026-05-07 — K=0 直接测量".

    ``CenterUV`` 跟踪 radial distortion 中心, 用物理 sensor 维度归一化, 因为材质
    的 ``CenterUV`` 参数住在归一化输出 UV 空间.
    """
...
def official_sensor_inverse_uv(
    ...
    *,
    ...
    center_uv
        Radial distortion center UV. It is one consumer of Disguise
        ``centerShiftMM``, but not the full projection effect. Path C also maps
        ``centerShiftMM`` to CineCamera ``Filmback.SensorHorizontalOffset`` /
        ``Filmback.SensorVerticalOffset`` via ``map_center_shift_projection``.
    aspect
