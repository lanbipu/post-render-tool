"""Path A · LensFile + BrownConradyUD M_RAT6 distortion math (ARCHIVED).

⚠️ 本模块已下架, 不再属于 plugin runtime. 仅作为 git 历史外可读的快照, 方便
未来需要回退到 Path A (LensFile-based distortion) 时直接复制回去.

回退步骤见 ../README.md.

模块职责: 把 CSV K1/K2/K3 翻译成 UE LensFile 的 8 系数 + fxfy + cxcy.
M_RAT6 rational form 拟合详见下方常量注释.
...
# ⚠️ 回退时把本文件复制到 Content/Python/post_render_tool/, 同时保证
# distortion_math.py 里也有 Path A 函数, 或者直接把本文件改名为
# distortion_math.py 后跟 Path C 部分合并.
