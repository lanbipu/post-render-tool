"""K=0 centerShift closed-loop validation: import 5 CSVs + render MRQ frames.

Runs inside an open UE Editor through remote_execution. Imports the 5
``path_c_center_k_zero_*`` CSVs (K1=K2=K3=0 + centerShiftMM ∈ {(0,0),
(±0.5,0), (0,±0.5)}) into a duplicated /Game/Main, then dispatches one PNG
MRQ frame per case for Mac-side phase-correlation against the original D3
PNGs.

Verifies the new sensor-mm centerShift formula
(``distortion_math.map_center_shift_projection``, 2026-05-07) end-to-end:
production import → LevelSequence with Filmback.Sensor*Offset tracks → MRQ
render. Mac compares each rendered PNG against the canonical D3 PNG and
expects < 1 px residual on the corresponding axis.
"""
...
REMOTE_CANONICAL_ROOT = Path("C:/temp/ue-remote/path_c_d3_exports/canonical/center_shift_k_zero")
REPORT_JSON = Path("C:/temp/ue-remote/path_c_k_zero_validation.json")
OUT_ROOT = Path("C:/temp/ue-remote/path_c_k_zero_validation")

SOURCE_MAP = "/Game/Main"
ASSET_ROOT = "/Game/PathCKZeroValidation"
RENDER_MAP = f"{ASSET_ROOT}/PathCKZero_Main"
...
ORDERED_CASES = (
    "path_c_center_k_zero_shift_zero",
    "path_c_center_k_zero_shiftx_n0p5",
    "path_c_center_k_zero_shiftx_p0p5",
    "path_c_center_k_zero_shifty_n0p5",
    "path_c_center_k_zero_shifty_p0p5",
)
...
REQUIRED_PROJECTION_TRACKS = {
    "SensorHorizontalOffset", "SensorVerticalOffset",
}
...
def _reset_render_assets() -> None:
    # `does_asset_exist` 在 UE 5.7 某些 build 上对 package path "/Game/Main" 返回 False,
    # 但 `find_asset_data` + `load_level` 都正常. 用 find_asset_data 做存在性探测.
    asset_data = unreal.EditorAssetLibrary.find_asset_data(SOURCE_MAP)
...
def _case_csvs() -> list[tuple[str, Path]]:
    paths: list[tuple[str, Path]] = []
    for case_id in ORDERED_CASES:
        path = REMOTE_CANONICAL_ROOT / f"{case_id}.csv"
        if not path.exists():
            raise RuntimeError(f"missing case CSV: {path}")
...
def _configure_job(queue, case_id: str, sequence_path: str) -> str:
    ...
    job.job_name = f"PathCKZero_{case_id}"
...
def run() -> dict[str, object]:
    ...
    try:
        ...
        # 模块缓存清理：UE Editor 跨多次 remote_execution 调用会缓存 plugin 模块，
        # 公式刚改完必须强制重载。
        import importlib
        ...
        try:
            for case_id, csv_path in _case_csvs():
