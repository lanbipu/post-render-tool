...
The plugin is exercised inside UE 5.7 Editor on `lanPC` (Windows 11 workstation). SSH alias is already configured in `~/.ssh/config` via the global `AGENTS.md`.
...
### Current Sources
- `developer-disguise-one` (8 files) → `.Codex/knowledge/developer-disguise-one/`
