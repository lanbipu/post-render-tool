...
│   │   ├── PostRenderToolWidget.h      ← 39 个 UPROPERTY 声明（31 Required + 8 Optional）
...
│   ├── bindwidget-contract.md          ← 39 控件契约速查
...
1. **契约层** — `PostRenderToolWidget.h` 用 39 个 `UPROPERTY` 声明（31 Required + 8 Optional）告诉 UE
...
│  Section 4 · Coordinate Verification       │  ← 单帧 Designer / UE 坐标预览对照
...
- 闭环验证流：调 Section 5 的轴映射 → 看 Section 4 四个数字变化 → 与现场真值对齐。

**设计原理**：`POSITION_MAPPING = {"x": (2, -100.0), ...}` 是从"Designer Y-up 米 → UE Z-up 厘米"理论推导得出，但 Disguise 导出坐标经常受 stage 校准偏移、导演镜像需求等影响偏 180°。把单帧的 Designer 原值与转换结果并排显示，让用户在拨动 Section 5 轴映射时能**所见即所得地**对照真值，避免在脑子里做矩阵乘法。
...
**设计原理**：轴映射的本质是 6 组 `(source_axis_index, scale_factor)` 元组，可表达绝大多数"右手/左手系 + 单位换算 + 方向翻转"。用 `ComboBox × SpinBox` 暴露给用户，比让他们手编 `config.py` 字典对现场 TD 更友好。`Apply`（内存沙盒）与 `Save`（固化到源码）是**双层承诺模型**——用户可反复拨动、配合 Section 4 的 Designer / UE 坐标预览对照直到对上，再一键固化，避免"改了忘存"。
...
**统一原则**：任何"不需要 Python 读或写的文字"都不要出现在 `_REQUIRED_CONTROLS` 里。控件数量保持 **31 Required + 8 Optional = 39**，与 `PostRenderToolWidget.h` 严格一致。

---

### 2.11 39 个 UPROPERTY 速查表（按 Figma 分节）
...
合计 **31 Required + 8 Optional = 39**。这张表与 `docs/bindwidget-contract.md` 冗余但本文档独立可读，两者必须同步更新。
...
**`save_axis_mapping` 的原子写入（`:315-426`）：** 详见 Phase 2 · Section 5（Save to config.py 的七步流程）。函数末尾的 `importlib.reload(config)` 让内存与磁盘同步。
...
**结论：** `PostRenderToolUI` 是整个 UI 层的核心，持有所有 UI 状态（当前 CSV 路径、解析结果、最后一次导入结果），并将 C++ UPROPERTY 声明的 39 个控件引用与 Python 回调绑定。
...
**结论：** 这个文件是整个项目的"类型接口文档"——它用 C++ 类型系统正式声明了"UI 面板应该有哪 39 个控件"。Blueprint 必须满足这份契约，Python 通过这份契约拿到控件引用，Figma 的命名也按这份契约对齐。
...
C++ 头文件用 `// ===== Section N: ... =====` 注释把 UPROPERTY 按 Figma 分节分组——Section 1 (Prerequisites) / Section 2 (CSV File) / Section 3 (CSV Preview) / Section 4 (Coord Verification) / Section 5a+5b (Axis Mapping Position + Rotation) / Section 6 (Actions + Results)。这让文件与 Figma 面板、Phase 2 的文档、BP 的视觉分节、JSON 规约形成**五方对齐**。
...
1. **Figma**：在 Section 6 RESULTS 区域画一个新的 `txt_anomaly_count` 文本节点，命名严格用此字符串
...
3. **JSON spec**：`docs/widget-tree-spec.json` 在 Section 6 节点的 children 里追加
...
使用真实的 Disguise Designer CSV 数据，通过 Phase 2 · Section 4 的工作流（拨动 `spn_frame` 看预览 → 调 Section 5 的轴映射）对比 `txt_designer_pos` 与 `txt_ue_pos`，手动调整轴映射直到 UE 坐标与现场期望值一致（若需目视确认，可在视口里手动 spawn 一个 `CineCameraActor` 验证）。记录正确的映射规则，按 `Save to config.py` 持久化，并补充一条 `# Verified with real data on YYYY-MM-DD` 注释。
