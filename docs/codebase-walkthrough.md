...
│   │   ├── PostRenderToolWidget.h      ← 41 个 UPROPERTY 声明（33 Required + 8 Optional）
...
│   ├── bindwidget-contract.md          ← 41 控件契约速查
...
1. **契约层** — `PostRenderToolWidget.h` 用 41 个 `UPROPERTY` 声明（33 Required + 8 Optional）告诉 UE
...
**`spawn_test_camera`（`:252-313`）：** 被 Phase 2 · Section 4 的 Spawn Test Camera 按钮驱动。核心是**按 Tag 而不是按 label** 追踪"测试相机"身份，避免误删用户自己创建的同名相机。流程：创建新相机 → 配置 Filmback → 删除携带 `PostRenderTool_TestCam` Tag 的旧相机 → `pilot_level_actor` 把 viewport 切到这台相机。
