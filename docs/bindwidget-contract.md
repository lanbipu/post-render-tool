...
└─ VerticalBox                         [装饰] lbl_prereq_vbox
   └─ ExpandableArea                   [装饰] expandable_prereqs               IsExpanded=true, HeaderPadding=0,0,0,0, AreaPadding=0,8,0,0, MaxHeight=0（=无限）; 点击 header 自动切换
      ├─ HorizontalBox                 [装饰] lbl_prereq_header                填入 HeaderContent slot（约定 children[0]）；自身无点击逻辑，只负责排列 4 个子控件
      ...
      └─ VerticalBox                   [装饰] lbl_prereq_body                  填入 BodyContent slot（约定 children[1]）；Visibility 由 ExpandableArea 原生控制，无需任何 BP 节点
