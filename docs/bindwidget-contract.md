...
折叠交互走 **UMG 原生 `UExpandableArea`**：控件自带"点 header 切换展开/收起"+ 箭头指示图标切换，**零 BP 事件连线**，且 header/body 完全可自定义。详见 [§7 方案 1](#7-折叠头-vs-btn_recheck常见混淆)。

**`UExpandableArea` 只认两个 named slot**（见 `ExpandableArea.cpp:18-19`，slot name 固定为 `"Header"` / `"Body"`）：

| slot | 承载 widget | 作用 |
|---|---|---|
| `HeaderContent` | `lbl_prereq_header`（`HorizontalBox`） | 始终可见的折叠头（点击触发切换 + 自动画箭头 ▶/▼） |
| `BodyContent` | `lbl_prereq_body`（`VerticalBox`） | 展开时显示的内容区（收起时 `Visibility=Collapsed` 由控件内部切换） |

JSON spec 用 `expandable_prereqs.children` 数组的**顺序**约定谁进哪个 slot：`children[0]` → Header，`children[1]` → Body。`spec_loader.py` 在 validate 阶段强制数组必须恰好 2 项。

```
Border                                 [装饰] lbl_card_prereq                  BrushColor=#242424, Padding=12,10,12,10
└─ VerticalBox                         [装饰] lbl_prereq_vbox
   └─ ExpandableArea                   [装饰] expandable_prereqs               IsExpanded=true, HeaderPadding=0,0,0,0, AreaPadding=0,8,0,0, MaxHeight=0（=无限）

      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      【HeaderContent slot】 — 点击即触发折叠，ExpandableArea 在左侧自动画 ▶/▼
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ├─ HorizontalBox                 [装饰] lbl_prereq_header                横排 4 个子控件
      │  ├─ SizeBox                    [装饰] lbl_prereq_accent                WidthOverride=3, HeightOverride=13, VAlign=Center, Padding=0,0,8,0
      ...
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      【BodyContent slot】 — 展开时显示；收起时由 ExpandableArea 原生 Collapse
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      └─ VerticalBox                   [装饰] lbl_prereq_body                  纵排 6 行状态 + 1 行按钮
...
**Header 归属（4 + 1 个控件）**：`lbl_prereq_header` · `lbl_prereq_accent` → `lbl_prereq_accent_img` · `lbl_prereq_title` · `lbl_prereq_header_spacer` · `prereq_summary`

**Body 归属（7 大块，共 22 个控件）**：
- `lbl_prereq_body`（容器）
- 6 × `lbl_prereq_row_N` + `lbl_prereq_dot_N` / `lbl_prereq_dot_N_img` / `prereq_label_N`（N = 0..5）
- `lbl_prereq_bottom_row` → `btn_recheck` → `lbl_btn_recheck_text`
...
| 方案 | 控件 | 契约 | 特点 | 匹配度 |
|---|---|---|---|---|
| **1. `UExpandableArea`**（✅ 当前采用） | 原生折叠控件 | 装饰 | 零 BP 连线；自带箭头图标 + 点击切换；HeaderContent / BodyContent 可塞任意 widget | 箭头默认是 UE 内置三角（`▶/▼` 视觉等效），想要自定义图标可替换 `Style.CollapsedImage` / `ExpandedImage` 的 brush |
| 2. `UButton` 包住 header | 装饰 Button + HorizontalBox | 装饰 | BP `OnClicked` 手写：翻转 bool → 切 body.Visibility + arrow.SetText(`▶/▼`） | 完全可控，但要 6 个 BP 节点 + 自建 bool 变量 |
...
**为什么选方案 1**：Section 1 的折叠是纯视觉行为，Python 不需要感知用户是否展开。`UExpandableArea` 把"点 header → 翻转展开态 → 箭头图标跟着切"这套逻辑压进 C++，比在 BP Event Graph 里手写 6 个节点更稳。若哪天需要"业务代码控制折叠态"，再切方案 3（代价仅是 C++ 契约加一行 + Editor 重启）。

### 7.1 方案 1 实现细节（`UExpandableArea` 配置）

**JSON spec 约定**（`docs/widget-tree-spec.json`）：ExpandableArea 节点的 `children` 数组**必须恰好 2 项**，`children[0]` → `HeaderContent` slot，`children[1]` → `BodyContent` slot。顺序颠倒会让 header 塞到 body 去。`spec_loader.py::EXPANDABLE_TYPES` 在 validate 阶段对这条约定做硬性检查。

**自动生成（`run_build()` 首次或重建）**：
`build_widget_blueprint.py::_ensure_expandable_slot` 调 C++ helper 的 **`EnsureWidgetInNamedSlot` UFUNCTION**（`PostRenderToolBuildHelper.h/.cpp` 新增）。C++ 侧：

1. 检查 `FindWidgetByNameRecursive`（现已支持下钻 `INamedSlotInterface` 的所有 slot）
2. 若 widget 已在正确 slot 内 → `AlreadyExisted`，Python 保留用户手调
3. 若 widget 存在但挂在其他父物体下（**迁移场景**：旧 spec 把 `lbl_prereq_header` 直接挂在 `lbl_prereq_vbox` 下）→ 自动 `DetachFromAnyParent` + `SetContentForSlot(SlotName, Existing)` 把它重新塞到 ExpandableArea 的指定 slot
4. 都不存在 → `Tree->ConstructWidget<>()` 创建 + `SetContentForSlot` 安放

**为什么要走 C++ 而不是纯 Python**：`UExpandableArea` 的 `HeaderContent` / `BodyContent` 是**裸 `UPROPERTY()`**（无 `BlueprintReadOnly` / 无 `EditAnywhere`），Python 反射看不见；`SetContentForSlot(FName, UWidget*)` 是 `INamedSlotInterface` 的裸 `virtual`，不是 `UFUNCTION`，Python 也调不到。唯一合法入口是 C++ 里 `Cast<INamedSlotInterface>(widget)->SetContentForSlot(...)`，因此我们必须在 `PostRenderToolBuildHelper` 里加一个 UFUNCTION 把这一步桥出去。

**ExpandableArea 可配属性**（已接入 `widget_properties.py`）：

| 属性 key | UE 内部字段 | 作用 |
|---|---|---|
| `IsExpanded` | `is_expanded` (bool) | 默认是否展开 |
| `HeaderPadding` | `header_padding` (FMargin) | header 外边距 |
| `AreaPadding` | `area_padding` (FMargin) | body 外边距 |
| `MaxHeight` | `max_height` (float) | body 最大高，`0` 表示无限 |

**样式自定义（可选）** — 如果想把默认三角箭头换成 Figma 指定的图标：
1. Designer 里选中 `expandable_prereqs` → Details → `Style` 展开
2. `Collapsed Image` / `Expanded Image` 下的 `Image` slot 里拖入你的 PNG 或 Material
3. 同时改 `Image Size` 到目标像素 — 项目里推荐 12×12

**BP 侧**：不需要任何 Event Graph 连线，Compile 保存就能用。

**首次启用 / 迁移步骤**（从方案 2 / 旧扁平 spec 切过来）：

1. **关闭 UE Editor**（UFUNCTION 变更不走 Live Coding，必须 UBT 全量重建 —— 见 `CLAUDE.md::Gotchas`）
2. 重新编译插件：IDE (Xcode/Rider) 的 "Build"，或 `xcodebuild -project ... -target PostRenderToolEditor`
3. 重开宿主项目；验证 `help(unreal.PostRenderToolBuildHelper)` 能看到 `ensure_widget_in_named_slot`
4. Python console 热重载：
   ```python
   import importlib
   from post_render_tool import spec_loader, widget_properties, build_widget_blueprint
   for m in (spec_loader, widget_properties, build_widget_blueprint):
       importlib.reload(m)
   build_widget_blueprint.run_build()
   ```
5. `run_build()` 会自动检测旧 BP 里 `lbl_prereq_header` / `lbl_prereq_body` 还挂在 `lbl_prereq_vbox` 下 → 创建新 `expandable_prereqs` → **自动把 header / body 迁移到 ExpandableArea 的 Header / Body slot**（不重建、保留用户的 Designer 样式调教）
6. Compile（Ctrl+Shift+F7）+ Save（Ctrl+S）

若迁移后出现异常（例如旧 BP 被手工改得非常乱），可走核选项：`delete_widget() + rebuild_from_spec()` 从零重建（但会丢所有手调，慎用）。
