...
- The Figma widget root uses `VerticalBox` -> `Border` -> `EditorUtilityScrollBox` -> `VerticalBox`. The outer `Border` is assigned a `VerticalBoxSlot` with `size_rule=Fill`, matching the legacy Editor Utility Widget pattern that gives the scroll panel a constrained viewport while still stretching with the tab.
